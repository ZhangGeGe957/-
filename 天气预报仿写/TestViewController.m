...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == 0) {
        ...
        for (int i = 0; i < self.weakDayArray.count; i++) {
            ...
            cell.maxLabel.text = self.weakmaxArray[i];
            ...
            self.imageView = [[UIImageView alloc] init];
            self.imageName = [NSString stringWithFormat:@"%@.png", self.weakImageArray[i]];
            self.imageView.image = [UIImage imageNamed:_imageName];
...
- (void)creatUrl {
    ...
    NSURLSessionTask *task1 = [session1 dataTaskWithURL:url completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        if (!error) {
            ...
            [self->_allInformationArray addObject:self.weakImageArray];
            [self->_allInformationArray addObject:self.weakmaxArray];
